...
using ClawOS.Application.CronJobs;
using ClawOS.Application.Llm;
...
using ClawOS.Channels.Telegram.Extensions;
using ClawOS.Domain.CronJobs.Repositories;
using ClawOS.Infrastructure.Configuration;
using ClawOS.Infrastructure.Llm.Ollama;
using ClawOS.Infrastructure.Llm.OpenAI;

using Serilog;

namespace ClawOS.Hosting;

public static class ServiceCollectionExtensions
{
    // Logging
    public static IServiceCollection AddClawOS(this IServiceCollection services, IConfiguration configuration)
    {
        ...
        services.AddScoped<ICronJobRepository, ClawOS.Infrastructure.CronJobs.Persistence.CronJobRepository>();
        services.AddScoped<ICronJobExecutionRepository, ClawOS.Infrastructure.CronJobs.Persistence.CronJobExecutionRepository>();
        services.AddScoped<IToolInstanceRepository, ClawOS.Infrastructure.CronJobs.Persistence.ToolInstanceRepository>();
    ...
    private static IServiceCollection AddSkillsFromAssemblies(this IServiceCollection services, IConfiguration configuration)
    {
        var assemblies = AppDomain.CurrentDomain.GetAssemblies()
            .Where(a => a.FullName?.StartsWith("ClawOS.Tools") == true)
            .ToList();

        var basePath = AppDomain.CurrentDomain.BaseDirectory;
        var skillDlls = Directory.GetFiles(basePath, "ClawOS.Tools.*.dll");
