    ...
    int maxOutputLength = 1000) : AgentToolBase<ExecuteCommandArgs>
{
    ...
    public override async Task<ToolResult> ExecuteAsync(ExecuteCommandArgs args, ToolContext context, CancellationToken ct)
    {
        ...
        {
            workingDirectory = ClawOS.Tools.FileSystem.PathSecurity.ResolveUserPath(
                args.WorkingDirectory, userId, context.IsSuperAdmin);
            var pathError = ClawOS.Tools.FileSystem.PathSecurity.ValidatePath(
        ...
        {
            workingDirectory = ClawOS.Tools.FileSystem.PathSecurity.GetUserWorkspacePath(userId);
        }
        ...
        {
            return ToolResult.Failure("Command execution timed out.");
        }
        catch (Exception ex)
        {
