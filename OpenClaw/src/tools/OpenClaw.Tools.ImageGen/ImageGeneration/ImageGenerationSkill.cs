...
namespace OpenClaw.Tools.ImageGen.ImageGeneration;

public class ImageGenerationSkill : AgentToolBase<ImageGenerationSkillArgs>
{
    ...
    public override async Task<ToolResult> ExecuteAsync(ImageGenerationSkillArgs args, CancellationToken ct)
    {
        var apiKey = Environment.GetEnvironmentVariable("OPENAI_API_KEY");

        if (string.IsNullOrWhiteSpace(apiKey))
            return ToolResult.Failure("OPENAI_API_KEY environment variable is not set.");

        if (string.IsNullOrWhiteSpace(args.Prompt))
            return ToolResult.Failure("Prompt is required for image generation.");
        ...
        try
        {
            ...
            if (response?.Value == null)
                return ToolResult.Failure("No image was generated.");
        ...
        {
            return ToolResult.Failure($"Image generation failed: {ex.Message}");
        }
    }
