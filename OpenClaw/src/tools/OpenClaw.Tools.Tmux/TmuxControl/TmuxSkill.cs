...
namespace OpenClaw.Tools.Tmux.TmuxControl;

public class TmuxSkill : AgentToolBase<TmuxSkillArgs>
{
    ...
    public override async Task<ToolResult> ExecuteAsync(TmuxSkillArgs args, CancellationToken ct)
    {
        var tmuxPath = await FindTmuxPathAsync(ct);
        if (tmuxPath == null)
            return ToolResult.Failure("tmux is not installed or not in PATH.");
        ...
        try
        {
            return args.Operation.ToLowerInvariant() switch
            {
                ...
                _ => ToolResult.Failure($"Unknown operation: {args.Operation}. Valid: list, new, kill, send, capture, list_windows, has_session")
            };
        }
        catch (Exception ex)
        {
            return ToolResult.Failure($"Tmux error: {ex.Message}");
        }
    }
    ...
    private static async Task<ToolResult> RunTmuxCommandAsync(string arguments, CancellationToken ct)
    {
        var psi = new ProcessStartInfo
        {
            FileName = "tmux",
        ...
        catch (OperationCanceledException)
        {
            ...
            return ToolResult.Failure("Tmux command timed out after 30 seconds.");
        }
        ...
        if (process.ExitCode != 0)
        {
            ...
            if (errorMessage.Contains("no server running"))
                return ToolResult.Success("No tmux sessions running.");
    ...
    private static Task<ToolResult> ListSessionsAsync(CancellationToken ct)
    {
        return RunTmuxCommandAsync("list-sessions -F '#{session_name}: #{session_windows} windows (created #{session_created_string})'", ct);
    }

    private static async Task<ToolResult> NewSessionAsync(string? sessionName, string? command, CancellationToken ct)
    {
        if (string.IsNullOrWhiteSpace(sessionName))
            return ToolResult.Failure("sessionName is required for new operation.");
        ...
        if (result.IsSuccess)
            return ToolResult.Success($"Session '{sessionName}' created successfully.");

        return result;
    }

    private static async Task<ToolResult> KillSessionAsync(string? sessionName, CancellationToken ct)
    {
        if (string.IsNullOrWhiteSpace(sessionName))
            return ToolResult.Failure("sessionName is required for kill operation.");

        var result = await RunTmuxCommandAsync($"kill-session -t {EscapeArg(sessionName)}", ct);
        if (result.IsSuccess)
            return ToolResult.Success($"Session '{sessionName}' killed successfully.");

        return result;
    }

    private static async Task<ToolResult> SendKeysAsync(string? sessionName, string? keys, string? window, string? pane, CancellationToken ct)
    {
        if (string.IsNullOrWhiteSpace(sessionName))
            return ToolResult.Failure("sessionName is required for send operation.");

        if (string.IsNullOrWhiteSpace(keys))
            return ToolResult.Failure("keys is required for send operation.");
        ...
        if (result.IsSuccess)
            return ToolResult.Success($"Keys sent to '{target}' successfully.");

        return result;
    }

    private static Task<ToolResult> CapturePaneAsync(string? sessionName, string? window, string? pane, int? lines, CancellationToken ct)
    {
        if (string.IsNullOrWhiteSpace(sessionName))
            return Task.FromResult(ToolResult.Failure("sessionName is required for capture operation."));
    ...
    private static Task<ToolResult> ListWindowsAsync(string? sessionName, CancellationToken ct)
    {
        if (string.IsNullOrWhiteSpace(sessionName))
            return Task.FromResult(ToolResult.Failure("sessionName is required for list_windows operation."));

        return RunTmuxCommandAsync($"list-windows -t {EscapeArg(sessionName)} -F '#{{window_index}}: #{{window_name}} (#{{window_panes}} panes)'", ct);
    }

    private static async Task<ToolResult> HasSessionAsync(string? sessionName, CancellationToken ct)
    {
        if (string.IsNullOrWhiteSpace(sessionName))
            return ToolResult.Failure("sessionName is required for has_session operation.");

        var result = await RunTmuxCommandAsync($"has-session -t {EscapeArg(sessionName)}", ct);

        if (result.IsSuccess)
            return ToolResult.Success($"Session '{sessionName}' exists.");

        if (result.Error?.Contains("can't find session") == true)
            return ToolResult.Success($"Session '{sessionName}' does not exist.");
