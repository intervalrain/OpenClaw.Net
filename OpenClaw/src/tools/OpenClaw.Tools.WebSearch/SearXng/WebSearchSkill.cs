...
namespace OpenClaw.Tools.WebSearch.SearXng;

public class WebSearchSkill(string? searxngUrl = null, TimeSpan? timeout = null) : AgentToolBase<WebSearchArgs>
{
    ...
    public override async Task<ToolResult> ExecuteAsync(WebSearchArgs args, CancellationToken ct)
    {
        ...
        try
        {
            ...
            if (response is null)
            {
                return ToolResult.Failure("Failed to get search results.");
            }
        ...
        {
            return ToolResult.Failure("Search request timed out.");
        }
        catch (Exception ex)
        {
