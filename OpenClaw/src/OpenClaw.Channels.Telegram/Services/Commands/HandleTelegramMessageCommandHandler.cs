...
namespace OpenClaw.Channels.Telegram.Services.Commands;

public record HandleTelegramMessageCommand(ChannelMessageReceivedEvent Event) : ICommand<ErrorOr<HandleTelegramMessageResult>>;

public record HandleTelegramMessageResult;

public class HandleTelegramMessageCommandHandler(
    ILogger<HandleTelegramMessageCommandHandler> logger,
    ...
    IUnitOfWork uow) : Mediator.IRequestHandler<HandleTelegramMessageCommand, ErrorOr<HandleTelegramMessageResult>>
{
    ...
    public async ValueTask<ErrorOr<HandleTelegramMessageResult>> Handle(HandleTelegramMessageCommand request, CancellationToken cancellationToken)
    {
        var evt = request.Event;
        var chatId = long.Parse(evt.ExternalChatId);    
        var username = evt.ExternalUsername ?? evt.ExternalUserId;
        var text = evt.Content!.Trim();

        logger.LogInformation("Processing message from {Username} ({chatId}: {Text})", username, chatId, text.Length > 100 ? text[..100] : text);
        ...
        if (text.StartsWith('/'))
        {
            ...
            if (_commands.Contains(commandText))
            {
                ...
                return new HandleTelegramMessageResult();
            }
        }

        await client.SendChatAction(chatId, ChatAction.Typing, cancellationToken: cancellationToken);

        return await ProcessAgentMessageAsync(chatId, username, text, cancellationToken);
    }
        ...
        switch (command)
        {
            ...
            case "help":
                await SendSafeAsync(chatId,
                    ...
                    "/`skill\\name` `args \\- Execute a skill drirectoy", cancellationToken);
                break;
            case "skills":
    ...
    private async Task<ErrorOr<HandleTelegramMessageResult>> ProcessAgentMessageAsync(long chatId, string username, string text, CancellationToken cancellationToken)
    {
        ...
        if (parser.TryParse(text, out var command))
        {
            ...
            if (skill is null)
            {
                ...
                return TelegramErrors.SkillNotFound(command.SkillName);
            }
            ...
            if (!await settings.IsEnabledAsync(command.SkillName, cancellationToken))
            {
                ...
                return TelegramErrors.SkillDisabled(command.SkillName);
            }
