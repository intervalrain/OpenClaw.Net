...
namespace OpenClaw.Skills.Notion.NotionCommand;
...
public class NotionSkill(
    ...
    ILogger<NotionSkill> logger) : AgentSkillBase<NotionSkillArgs>
{
    ...
    public override async Task<SkillResult> ExecuteAsync(NotionSkillArgs args, CancellationToken ct)
    {
        ...
        catch (HttpRequestException ex)
        {
            ...
            return SkillResult.Failure($"Notion API error: {ex.Message}");
        }
        catch (Exception ex)
        {
    ...
    private static async Task<SkillResult> SearchAsync(HttpClient client, NotionSkillArgs args, CancellationToken ct)
    {
    ...
    private static async Task<SkillResult> GetPageAsync(HttpClient client, NotionSkillArgs args, CancellationToken ct)
    {
        if (string.IsNullOrWhiteSpace(args.PageId))
            return SkillResult.Failure("PageId is required for get_page operation.");

        var pageId = NormalizeId(args.PageId);
        return await GetAsync(client, $"{BaseUrl}/pages/{pageId}", ct);
    }

    private static async Task<SkillResult> CreatePageAsync(HttpClient client, NotionSkillArgs args, CancellationToken ct)
    {
        if (string.IsNullOrWhiteSpace(args.ParentId))
            return SkillResult.Failure("ParentId (page or database ID) is required for create_page operation.");

        if (string.IsNullOrWhiteSpace(args.Title))
            return SkillResult.Failure("Title is required for create_page operation.");
    ...
    private static async Task<SkillResult> UpdatePageAsync(HttpClient client, NotionSkillArgs args, CancellationToken ct)
    {
        if (string.IsNullOrWhiteSpace(args.PageId))
            return SkillResult.Failure("PageId is required for update_page operation.");
    ...
    private static async Task<SkillResult> CreateDatabaseAsync(HttpClient client, NotionSkillArgs args, CancellationToken ct)
    {
        if (string.IsNullOrWhiteSpace(args.ParentId))
            return SkillResult.Failure("ParentId (page ID) is required for create_database operation.");

        if (string.IsNullOrWhiteSpace(args.Title))
            return SkillResult.Failure("Title is required for create_database operation.");
        ...
        if (!string.IsNullOrWhiteSpace(args.SchemaJson))
        {
            ...
            {
                return SkillResult.Failure($"Invalid schema JSON: {ex.Message}");
            }
        }
    ...
    private static async Task<SkillResult> QueryDatabaseAsync(HttpClient client, NotionSkillArgs args, CancellationToken ct)
    {
        if (string.IsNullOrWhiteSpace(args.DatabaseId))
            return SkillResult.Failure("DatabaseId is required for query_database operation.");
        ...
        if (!string.IsNullOrWhiteSpace(args.FilterJson))
        {
            ...
            {
                return SkillResult.Failure($"Invalid filter JSON: {ex.Message}");
            }
        }
    ...
    private static async Task<SkillResult> ListDatabasesAsync(HttpClient client, CancellationToken ct)
    {
        var body = new Dictionary<string, object>
        {
    ...
    private static async Task<SkillResult> GetDatabaseAsync(HttpClient client, NotionSkillArgs args, CancellationToken ct)
    {
        if (string.IsNullOrWhiteSpace(args.DatabaseId))
            return SkillResult.Failure("DatabaseId is required for get_database operation.");

        var databaseId = NormalizeId(args.DatabaseId);
        return await GetAsync(client, $"{BaseUrl}/databases/{databaseId}", ct);
    }

    private static async Task<SkillResult> AddCommentAsync(HttpClient client, NotionSkillArgs args, CancellationToken ct)
    {
        if (string.IsNullOrWhiteSpace(args.PageId))
            return SkillResult.Failure("PageId is required for add_comment operation.");

        if (string.IsNullOrWhiteSpace(args.Content))
            return SkillResult.Failure("Content is required for add_comment operation.");
    ...
    private static async Task<SkillResult> GetBlockChildrenAsync(HttpClient client, NotionSkillArgs args, CancellationToken ct)
    {
        if (string.IsNullOrWhiteSpace(args.PageId) && string.IsNullOrWhiteSpace(args.BlockId))
            return SkillResult.Failure("PageId or BlockId is required for get_block_children operation.");
    ...
    private static async Task<SkillResult> GetAsync(HttpClient client, string url, CancellationToken ct)
    {
        ...
        {
            return SkillResult.Failure($"API error ({response.StatusCode}): {TruncateError(content)}");
        }

        return SkillResult.Success(FormatJson(content));
    }

    private static async Task<SkillResult> PostAsync(HttpClient client, string url, object body, CancellationToken ct)
    {
        ...
        if (!response.IsSuccessStatusCode)
        {
            return SkillResult.Failure($"API error ({response.StatusCode}): {TruncateError(responseContent)}");
        }

        return SkillResult.Success(FormatJson(responseContent));
    }

    private static async Task<SkillResult> PatchAsync(HttpClient client, string url, object body, CancellationToken ct)
    {
        var json = JsonSerializer.Serialize(body, JsonOptions);
        var content = new StringContent(json, Encoding.UTF8, "application/json");
