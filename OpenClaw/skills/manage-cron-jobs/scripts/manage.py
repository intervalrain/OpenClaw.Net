...
Called by the manage-cron-jobs skill to interact with the OpenClaw API.
Reads auth token from environment or ~/.openclaw/auth.json.
...
BASE_URL = os.environ.get("OPENCLAW_URL", "http://localhost:5002")
...
def get_token():
    ...
    auth_file = os.path.expanduser("~/.openclaw/auth.json")
    if os.path.exists(auth_file):
        with open(auth_file) as f:
            data = json.load(f)
            return data.get("token")

    print("Error: No auth token. Set OPENCLAW_TOKEN env var or login via UI.", file=sys.stderr)
    sys.exit(1)
