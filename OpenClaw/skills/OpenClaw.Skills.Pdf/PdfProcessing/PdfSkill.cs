...
namespace OpenClaw.Skills.Pdf.PdfProcessing;

public class PdfSkill : AgentSkillBase<PdfSkillArgs>
{
    ...
    public override async Task<SkillResult> ExecuteAsync(PdfSkillArgs args, CancellationToken ct)
    {
        if (string.IsNullOrWhiteSpace(args.FilePath))
            return SkillResult.Failure("filePath is required.");

        if (!File.Exists(args.FilePath))
            return SkillResult.Failure($"File not found: {args.FilePath}");

        if (!args.FilePath.EndsWith(".pdf", StringComparison.OrdinalIgnoreCase))
            return SkillResult.Failure("File must be a PDF (.pdf extension).");
        ...
        try
        {
            var result = args.Operation.ToLowerInvariant() switch
            {
                ...
                _ => SkillResult.Failure($"Unknown operation: {args.Operation}. Valid: read, metadata, page_count, page, search")
            };
        ...
        {
            return SkillResult.Failure($"PDF processing error: {ex.Message}");
        }
    }

    private static Task<SkillResult> ExtractTextAsync(
    ...
    private static Task<SkillResult> GetMetadataAsync(string filePath, CancellationToken ct)
    {
        return Task.Run(() =>
        {
            ct.ThrowIfCancellationRequested();
    ...
    private static Task<SkillResult> GetPageCountAsync(string filePath, CancellationToken ct)
    {
        return Task.Run(() =>
        {
            ct.ThrowIfCancellationRequested();
            using var document = PdfDocument.Open(filePath);
            return SkillResult.Success($"Page count: {document.NumberOfPages}");
        }, ct);
    }

    private static Task<SkillResult> GetPageTextAsync(string filePath, int? pageNumber, CancellationToken ct)
    {
        return Task.Run(() =>
        {
            ct.ThrowIfCancellationRequested();
            ...
            if (pageNumber == null || pageNumber < 1)
                return SkillResult.Failure("pageNumber is required and must be >= 1.");

            using var document = PdfDocument.Open(filePath);

            if (pageNumber > document.NumberOfPages)
                return SkillResult.Failure($"Page {pageNumber} does not exist. Document has {document.NumberOfPages} pages.");
    ...
    private static Task<SkillResult> SearchTextAsync(string filePath, string? searchTerm, CancellationToken ct)
    {
        return Task.Run(() =>
        {
            ct.ThrowIfCancellationRequested();
            ...
            if (string.IsNullOrWhiteSpace(searchTerm))
                return SkillResult.Failure("searchTerm is required for search operation.");
            ...
            if (results.Count == 0)
                return SkillResult.Success($"No matches found for '{searchTerm}' in the document.");
